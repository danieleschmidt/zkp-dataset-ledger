...
impl Storage for MemoryStorage {
    ...
    fn stats(&self) -> Result<StorageStats, LedgerError> {
        let total_size_bytes = self.data.values()
            .map(|v| v.len() as u64)
            .sum();
...
pub mod rocksdb_backend {
    ...
    impl RocksDbStorage {
        ...
        pub fn new_with_options<P: AsRef<Path>>(path: P, opts: Options) -> Result<Self, LedgerError> {
            let db = DB::open(&opts, path)
                .map_err(|e| LedgerError::StorageError(format!("RocksDB open failed: {}", e)))?;
    ...
    impl Storage for RocksDbStorage {
        ...
        fn list_keys(&self, prefix: &str) -> Result<Vec<String>, LedgerError> {
            ...
            for item in iter {
                let (key_bytes, _) = item
                    .map_err(|e| LedgerError::StorageError(format!("RocksDB iteration failed: {}", e)))?;
        ...
        fn exists(&self, key: &str) -> Result<bool, LedgerError> {
            match self.db.get(key.as_bytes()) {
                Ok(Some(_)) => Ok(true),
                Ok(None) => Ok(false),
                Err(e) => Err(LedgerError::StorageError(format!("RocksDB exists check failed: {}", e))),
            }
        }
        ...
        fn stats(&self) -> Result<StorageStats, LedgerError> {
            ...
            for item in iter {
                let (key, value) = item
                    .map_err(|e| LedgerError::StorageError(format!("RocksDB stats iteration failed: {}", e)))?;
...
pub mod postgres_backend {
    ...
    impl PostgresStorage {
        pub async fn new(database_url: &str) -> Result<Self, LedgerError> {
            let pool = PgPoolOptions::new()
                ...
                .await
                .map_err(|e| LedgerError::StorageError(format!("PostgreSQL connection failed: {}", e)))?;
            ...
            let rt = Runtime::new()
                .map_err(|e| LedgerError::StorageError(format!("Runtime creation failed: {}", e)))?;
    ...
    impl Storage for PostgresStorage {
        ...
        fn get(&self, key: &str) -> Result<Option<Vec<u8>>, LedgerError> {
            self.rt.block_on(async {
                let row = sqlx::query("SELECT value FROM ledger_storage WHERE key = $1")
                    ...
                    .await
                    .map_err(|e| LedgerError::StorageError(format!("PostgreSQL get failed: {}", e)))?;
        ...
        fn delete(&mut self, key: &str) -> Result<(), LedgerError> {
            self.rt.block_on(async {
                sqlx::query("DELETE FROM ledger_storage WHERE key = $1")
                    ...
                    .execute(&self.pool)
                    .await
                    .map_err(|e| LedgerError::StorageError(format!("PostgreSQL delete failed: {}", e)))?;
        ...
        fn list_keys(&self, prefix: &str) -> Result<Vec<String>, LedgerError> {
            self.rt.block_on(async {
                let rows = sqlx::query("SELECT key FROM ledger_storage WHERE key LIKE $1")
                    ...
                    .fetch_all(&self.pool)
                    .await
                    .map_err(|e| LedgerError::StorageError(format!("PostgreSQL list_keys failed: {}", e)))?;
        ...
        fn exists(&self, key: &str) -> Result<bool, LedgerError> {
            self.rt.block_on(async {
                let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM ledger_storage WHERE key = $1")
                    .bind(key)
                    .fetch_one(&self.pool)
                    .await
                    .map_err(|e| LedgerError::StorageError(format!("PostgreSQL exists check failed: {}", e)))?;
...
pub fn create_storage(backend_type: &str, config: &str) -> Result<Box<dyn Storage>, LedgerError> {
    match backend_type {
        ...
        _ => Err(LedgerError::StorageError(format!("Unknown storage backend: {}", backend_type))),
    }
}
