...
impl SecurityManager {
    ...
    pub fn track_suspicious_activity(&self, ip_address: &str, activity_type: &str) {
        ...
        if let Some(activity) = self.suspicious_activities.get(ip_address) {
            if activity.risk_score > 0.8 || activity.activity_count > 20 {
                ...
                log::warn!("Blocked suspicious IP: {} (risk: {:.2}, activities: {})", 
                    ip_address, activity.risk_score, activity.activity_count);
            }
        }
    }
    ...
    pub fn rotate_keys(&mut self) -> Result<()> {
        ...
        let current_version = keys.get("primary")
            .map(|k| k.version)
            .unwrap_or(0);
        ...
        log::info!("Successfully rotated encryption keys to version {}", current_version + 1);
        Ok(())
    }
