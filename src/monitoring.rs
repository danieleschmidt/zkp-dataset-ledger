    ...
    pub fn record_operation(&self, operation_type: &str, duration: Duration, success: bool) {
        ...
        // Update system metrics
        {
            ...
            metrics.avg_operation_duration_ms =
                (metrics.avg_operation_duration_ms * (total_ops - 1.0) + duration_ms as f64)
                    / total_ops;
    ...
    fn check_resource_alerts(&self, memory_bytes: u64, storage_bytes: u64, cpu_percent: f32) {
        ...
        if storage_bytes > self.alert_config.max_storage_usage_bytes {
            self.create_alert(
                ...
                "High Storage Usage",
                &format!(
                    "Storage usage {}GB exceeds threshold {}GB",
        ...
        match component {
            "ledger" => {
                ...
                (
                    ...
                    "Ledger operations normal".to_string(),
                )
            }
            "storage" => {
                ...
                (
                    HealthStatus::Healthy,
                    "Storage backend accessible".to_string(),
                )
            }
            "crypto" => {
                ...
                (
                    HealthStatus::Healthy,
                    "Cryptographic functions operational".to_string(),
                )
            }
            "network" => {
                ...
                (
                    HealthStatus::Healthy,
                    "Network connectivity normal".to_string(),
                )
            }
            _ => (
...
impl MetricsExporter {
    ...
    pub fn export_prometheus(report: &MonitoringReport) -> String {
        ...
        for (op_type, metrics) in &report.operation_metrics {
            output.push_str(&format!(
                "zkp_operation_count{{operation=\"{}\"}} {}\n",
                op_type, metrics.count
            ));
            output.push_str(&format!(
                "zkp_operation_duration_ms{{operation=\"{}\"}} {}\n",
                op_type, metrics.avg_duration_ms
            ));
            output.push_str(&format!(
                "zkp_operation_success_rate{{operation=\"{}\"}} {}\n",
    ...
    pub fn export_json(report: &MonitoringReport) -> Result<String> {
        serde_json::to_string_pretty(report).map_err(|e| LedgerError::Json(e))
    }
}
