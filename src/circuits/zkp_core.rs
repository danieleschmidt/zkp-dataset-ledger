...
impl ZKPCircuit {
    ...
    pub fn new(
        ...
        log::info!(
            "Creating ZKP circuit {} for dataset {}",
            circuit_id,
            dataset.name
        );
    ...
    fn generate_constraints(&mut self, properties: &[DatasetProperty]) -> Result<()> {
        for property in properties {
            match property {
                ...
                DatasetProperty::DataQuality {
                    ...
                    self.add_quality_constraints(
                        *completeness_ratio,
                        *uniqueness_ratio,
                        *validity_score,
                    )?;
                }
                DatasetProperty::Fairness {
                    protected_attribute,
                    parity_threshold,
                    bias_metrics,
                } => {
        ...
        log::info!(
            "Generated {} constraints for circuit {}",
        ...
        let constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::Integer(count as i64),
                );
            ...
            let type_constraint = CircuitConstraint {
                ...
                parameters: {
                    ...
                    params.insert(
                        ...
                        ConstraintValue::Integer(i as i64),
                    );
                    params.insert(
                        "expected_type".to_string(),
                        ConstraintValue::String(format!("{:?}", col_type)),
                    );
                    params
                },
            };
        ...
        let variance_constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::Float(variance_range.0),
                );
                params.insert(
                    "max_variance".to_string(),
                    ConstraintValue::Float(variance_range.1),
                );
                params
            },
        };
        ...
        let dist_constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::String(format!("{:?}", distribution_type)),
                );
                params
            },
        };
        ...
        let quality_constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::Float(completeness),
                );
    ...
    fn add_fairness_constraints(
        ...
        let fairness_constraint = CircuitConstraint {
            ...
            parameters: {
                let mut params = HashMap::new();
                params.insert(
                    "protected_attribute".to_string(),
                    ConstraintValue::String(protected_attribute.to_string()),
                );
                params.insert(
                    "parity_threshold".to_string(),
                    ConstraintValue::Float(parity_threshold),
                );

                // Add bias metrics
                let metrics_data: Vec<ConstraintValue> = bias_metrics
                    .iter()
                    .map(|m| {
                        ConstraintValue::String(format!(
                            "{}:{}:{}",
                            m.metric_name, m.value, m.threshold
                        ))
                    })
                    .collect();
                params.insert(
                    "bias_metrics".to_string(),
                    ConstraintValue::Array(metrics_data),
                );
                params
            },
        };
        ...
        let privacy_constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::Integer(anonymization_level as i64),
                );

                if let Some(k) = k_anonymity {
                    params.insert(
                        "k_anonymity".to_string(),
                        ConstraintValue::Integer(k as i64),
                    );
                }

                if let Some(l) = l_diversity {
                    params.insert(
                        "l_diversity".to_string(),
                        ConstraintValue::Integer(l as i64),
                    );
                }
        ...
        let custom_constraint = CircuitConstraint {
            ...
            parameters: {
                ...
                params.insert(
                    ...
                    ConstraintValue::Hash(proof_data.to_vec()),
                );
                params.insert(
                    "verification_key".to_string(),
                    ConstraintValue::String(verification_key.to_string()),
                );
                params
            },
        };
    ...
    pub fn generate_witness(&mut self, dataset: &Dataset) -> Result<CircuitWitness> {
        ...
        public_inputs.insert(
            ...
            ConstraintValue::Hash(dataset.hash.as_bytes().to_vec()),
        );

        // Add circuit ID as public input
        public_inputs.insert(
            "circuit_id".to_string(),
            ConstraintValue::String(self.circuit_id.clone()),
        );
            ...
            match property {
                ...
                DatasetProperty::Schema { columns, count } => {
                    ...
                    private_inputs.insert(
                        ...
                        ConstraintValue::Hash(self.hash_schema(columns)),
                    );
                }
                DatasetProperty::Statistics {
                    mean_range,
                    ...
                    public_inputs
                        .insert("mean_min".to_string(), ConstraintValue::Float(mean_range.0));
                    public_inputs
                        .insert("mean_max".to_string(), ConstraintValue::Float(mean_range.1));
                    public_inputs.insert(
                        "variance_min".to_string(),
                        ConstraintValue::Float(variance_range.0),
                    );
                    public_inputs.insert(
                        "variance_max".to_string(),
                        ConstraintValue::Float(variance_range.1),
                    );
                }
                DatasetProperty::DataQuality {
                    ...
                    public_inputs.insert(
                        "completeness_commitment".to_string(),
                        ConstraintValue::Float(*completeness_ratio),
                    );
                    public_inputs.insert(
                        "uniqueness_commitment".to_string(),
                        ConstraintValue::Float(*uniqueness_ratio),
                    );
                    public_inputs.insert(
                        "validity_commitment".to_string(),
                        ConstraintValue::Float(*validity_score),
                    );
                }
                DatasetProperty::Fairness {
                    parity_threshold,
                    ...
                    private_inputs.insert(
                        "bias_results".to_string(),
                        ConstraintValue::Array(
                            bias_metrics
                                .iter()
                                .map(|m| ConstraintValue::Boolean(m.passed))
                                .collect(),
                        ),
                    );
                }
                DatasetProperty::Privacy {
                    ...
                    public_inputs.insert(
                        "anon_level".to_string(),
                        ConstraintValue::Integer(*anonymization_level as i64),
                    );
                    if let Some(k) = k_anonymity {
                        public_inputs
                            .insert("k_anon".to_string(), ConstraintValue::Integer(*k as i64));
                    }
                    if let Some(l) = l_diversity {
                        public_inputs
                            .insert("l_div".to_string(), ConstraintValue::Integer(*l as i64));
                    }
                }
                DatasetProperty::Custom {
                    property_name,
                    ...
                    private_inputs.insert(
                        format!("custom_{}", property_name),
                        ConstraintValue::Hash(proof_data.clone()),
                    );
                }
            }
        }
    ...
    fn serialize_public_inputs(&self) -> Result<Vec<u8>> {
        let witness = self
            .witness
            .as_ref()
            .ok_or_else(|| LedgerError::invalid_input("witness", "No witness available"))?;
...
impl ZKPCircuitBuilder {
    ...
    pub fn with_schema(mut self, columns: Vec<ColumnType>) -> Self {
        ...
        self.properties
            .push(DatasetProperty::Schema { columns, count });
        self
    }
