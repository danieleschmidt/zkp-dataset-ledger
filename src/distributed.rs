...
impl DistributedProcessor {
    ...
    async fn join_cluster(&self) -> Result<()> {
        info!(
            "Joining cluster with {} bootstrap nodes",
            self.cluster_config.bootstrap_nodes.len()
        );
    ...
    pub async fn distribute_tasks(&self, tasks: Vec<DistributedTask>) -> Result<Vec<TaskResult>> {
        ...
        let task_partitions = self
            .load_balancer
            .partition_tasks(tasks, &available_nodes)
            .await?;
        ...
        for (node_id, node_tasks) in task_partitions {
            ...
            let handle = tokio::spawn(async move {
                task_scheduler
                    .execute_tasks_on_node(node_id, node_tasks, network_manager)
                    .await
            });
        ...
        for handle in task_handles {
            match handle.await {
                ...
                Err(e) => {
                    ...
                    return Err(LedgerError::ConcurrencyError(format!(
                        "Task join error: {}",
                        e
                    )));
                }
        ...
        info!(
            "Completed distributed processing of {} tasks",
            all_results.len()
        );
        Ok(all_results)
    }
    ...
    pub async fn distributed_proof_generation(
        &self,
        proof_requests: Vec<ProofRequest>,
    ) -> Result<Vec<ProofResult>> {
        info!(
            "Starting distributed proof generation for {} requests",
            proof_requests.len()
        );

        // Convert proof requests to distributed tasks
        let tasks: Vec<DistributedTask> = proof_requests
            .into_iter()
                ...
                data: serde_json::to_vec(&request)
                    .map_err(|e| {
                        LedgerError::Serialization(format!(
                            "Failed to serialize proof request: {}",
                            e
                        ))
                    })
                    .unwrap_or_default(),
        ...
        let proof_results: Vec<ProofResult> = task_results
            .into_iter()
        ...
        info!(
            "Distributed proof generation completed with {} results",
            proof_results.len()
        );
        Ok(proof_results)
    }
    ...
    fn calculate_resource_requirements(&self, request: &ProofRequest) -> ResourceRequirements {
        match request {
            ...
            ProofRequest::Transform { input_size, .. } => ResourceRequirements {
                cpu_cores: 2,
                memory_mb: (*input_size / 20_000).max(150).min(1500) as u32,
                storage_mb: 75,
                network_mb: 15,
            },
            ProofRequest::Custom { .. } => ResourceRequirements {
                cpu_cores: 1,
                memory_mb: 500,
                storage_mb: 100,
                network_mb: 10,
    ...
    async fn run_heartbeat_service(&self) -> Result<()> {
        let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(
            self.cluster_config.heartbeat_interval_seconds as u64,
        ));
    ...
    async fn send_heartbeat_to_cluster(&self) -> Result<()> {
        ...
        for node in nodes {
            if node.id != self.node_id {
                if let Err(e) = self
                    .network_manager
                    .send_heartbeat(&node.address, &heartbeat)
                    .await
                {
        ...
        if !failed_nodes.is_empty() {
            ...
            for failed_node in failed_nodes {
                let proposal = ConsensusProposal {
                    ...
                    proposal_type: ProposalType::RemoveNode {
                        node_id: failed_node,
                    },
                    timestamp: Utc::now(),
                };
    ...
    async fn request_cluster_membership(&self, bootstrap_address: &SocketAddr) -> Result<()> {
        ...
        self.network_manager
            .send_membership_request(bootstrap_address, &membership_request)
            .await
    }
...
impl NodeManager {
    ...
    pub async fn become_leader(&self) -> Result<()> {
        let mut is_leader = self.is_leader.write().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire leader lock".to_string())
        })?;
        *is_leader = true;
    ...
    pub async fn get_available_nodes(&self) -> Result<Vec<ClusterNode>> {
        let nodes = self.cluster_nodes.read().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire nodes lock".to_string())
        })?;

        let health = self.node_health.read().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire health lock".to_string())
        })?;

        let available_nodes: Vec<ClusterNode> = nodes
            .values()
            .filter(|node| {
                health
                    .get(&node.id)
    ...
    pub async fn detect_failed_nodes(&self) -> Result<Vec<Uuid>> {
        ...
        let failure_threshold =
            ChronoDuration::seconds(self.config.node_failure_timeout_seconds as i64);

        let failed_nodes: Vec<Uuid> = health
            .iter()
    ...
    pub async fn add_node(&self, node: ClusterNode) -> Result<()> {
        let mut nodes = self.cluster_nodes.write().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire nodes lock".to_string())
        })?;
...
impl TaskScheduler {
    ...
    pub async fn execute_tasks_on_node(
        &self,
        node_id: Uuid,
        ...
        for task in tasks {
            ...
            match network_manager.send_task_to_node(node_id, &task).await {
                ...
                Err(e) => {
                    error!(
                        "Failed to execute task {} on node {}: {}",
                        task.id, node_id, e
                    );
    ...
    pub async fn execute_local_task(&self, task: DistributedTask) -> Result<TaskResult> {
        ...
        let result = match task.task_type {
            TaskType::ProofGeneration => self.execute_proof_generation_task(&task).await,
            TaskType::DataValidation => self.execute_data_validation_task(&task).await,
            TaskType::Computation => self.execute_computation_task(&task).await,
    ...
    async fn execute_proof_generation_task(&self, task: &DistributedTask) -> Result<Vec<u8>> {
        ...
        let proof_request: ProofRequest = serde_json::from_slice(&task.data).map_err(|e| {
            LedgerError::Serialization(format!("Failed to deserialize proof request: {}", e))
        })?;
        ...
        // Serialize result
        serde_json::to_vec(&proof_result).map_err(|e| {
            LedgerError::Serialization(format!("Failed to serialize proof result: {}", e))
        })
    }
    ...
    async fn record_active_task(&self, task_id: Uuid, node_id: Uuid) -> Result<()> {
        let mut active_tasks = self.active_tasks.write().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire active tasks lock".to_string())
        })?;
    ...
    pub async fn get_active_task_count(&self) -> Result<u32> {
        let active_tasks = self.active_tasks.read().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to acquire active tasks lock".to_string())
        })?;
...
impl LoadBalancer {
    ...
    pub async fn partition_tasks(
        ...
        if nodes.is_empty() {
            return Err(LedgerError::InvalidInput(
                "No nodes available for task distribution".to_string(),
            ));
        }

        match self.config.strategy {
            LoadBalancingStrategy::RoundRobin => self.round_robin_partition(tasks, nodes).await,
            LoadBalancingStrategy::LeastLoaded => self.least_loaded_partition(tasks, nodes).await,
    ...
    async fn least_loaded_partition(
        &self,
        tasks: Vec<DistributedTask>,
        nodes: &[ClusterNode],
    ) -> Result<HashMap<Uuid, Vec<DistributedTask>>> {
        let mut partitions: HashMap<Uuid, Vec<DistributedTask>> = HashMap::new();
        let mut node_loads: HashMap<Uuid, u32> = nodes.iter().map(|node| (node.id, 0u32)).collect();

        for task in tasks {
            // Find least loaded node
            let least_loaded_node = node_loads
                .iter()
    ...
    async fn resource_aware_partition(
        &self,
        tasks: Vec<DistributedTask>,
        nodes: &[ClusterNode],
    ) -> Result<HashMap<Uuid, Vec<DistributedTask>>> {
        let mut partitions: HashMap<Uuid, Vec<DistributedTask>> = HashMap::new();
        let mut node_resources: HashMap<Uuid, (u32, u32)> = nodes
            .iter()
            .map(|node| {
                (
                    node.id,
    ...
    async fn locality_aware_partition(
        &self,
        tasks: Vec<DistributedTask>,
        nodes: &[ClusterNode],
    ) -> Result<HashMap<Uuid, Vec<DistributedTask>>> {
...
impl NetworkManager {
    ...
    pub async fn send_task_to_node(
        &self,
        _node_id: Uuid,
    ...
    pub async fn send_membership_request(
        &self,
        _address: &SocketAddr,
...
impl ConsensusManager {
    ...
    pub async fn get_cluster_state_version(&self) -> Result<u64> {
        let version = self.cluster_state_version.read().map_err(|_| {
            LedgerError::ConcurrencyError("Failed to read cluster state version".to_string())
        })?;
        Ok(*version)
    }
