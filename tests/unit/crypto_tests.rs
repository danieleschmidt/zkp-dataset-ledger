...
use crate::fixtures::{TestDataGenerator, PerformanceTester, constants};
...
fn test_hash_different_algorithms() {
    ...
    assert_ne!(sha3_hash, blake3_hash, "Different algorithms should produce different hashes");
}
...
fn test_merkle_tree_construction() {
    ...
    let tree = MerkleTree::new(leaves, "sha3-256")
...
fn test_merkle_tree_single_leaf() {
    ...
    let tree = MerkleTree::new(leaves, "sha3-256")
        .expect("Failed to construct single-leaf Merkle tree");
...
fn test_merkle_tree_proof_generation() {
    ...
    for i in 0..4 {
        ...
        let is_valid = tree.verify_proof(i, &proof)
...
fn test_merkle_tree_deterministic() {
    ...
    let tree1 = MerkleTree::new(leaves.clone(), "sha3-256")
        .expect("Failed to construct first tree");
    let tree2 = MerkleTree::new(leaves, "sha3-256")
        .expect("Failed to construct second tree");
...
fn test_merkle_tree_order_sensitivity() {
    ...
    let tree1 = MerkleTree::new(leaves1, "sha3-256")
        .expect("Failed to construct first tree");
    let tree2 = MerkleTree::new(leaves2, "sha3-256")
        .expect("Failed to construct second tree");
...
fn test_proof_generator_basic() {
    ...
    let proof_gen = ProofGenerator::new("groth16", "bls12-381")
        .expect("Failed to create proof generator");
    
    let timer = PerformanceTester::new();
    let proof = proof_gen.generate_row_count_proof(&dataset_path)
...
fn test_verifier_valid_proof() {
    ...
    let proof_gen = ProofGenerator::new("groth16", "bls12-381")
        .expect("Failed to create proof generator");
    let verifier = Verifier::new("groth16", "bls12-381")
...
fn test_verifier_invalid_proof() {
    ...
    let is_valid = verifier.verify(&invalid_proof)
