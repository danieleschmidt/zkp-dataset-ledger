...
use crate::fixtures::{TestDataGenerator, TestLedger};
...
pub fn bench_dataset_loading(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("csv_loading", size), size, |b, _| {
            b.iter(|| {
                let _dataset =
                    Dataset::from_path(black_box(&dataset_path)).expect("Failed to load dataset");
            });
        });
    }
...
pub fn bench_proof_verification(c: &mut Criterion) {
    ...
    let proof = ledger
        .notarize_dataset(dataset, "verification-benchmark", ProofConfig::default())
...
pub fn bench_merkle_tree_operations(c: &mut Criterion) {
    ...
    for leaf_count in [100, 1_000, 10_000, 100_000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("construction", leaf_count),
            leaf_count,
            |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("proof_generation", leaf_count),
            leaf_count,
            |b, _| {
                b.iter(|| {
...
pub fn bench_hash_functions(c: &mut Criterion) {
    ...
    for algorithm in algorithms.iter() {
        for size in data_sizes.iter() {
            ...
            let hasher =
                zkp_dataset_ledger::crypto::Hash::new(algorithm).expect("Failed to create hasher");
            ...
            rt.block_on(async {
                ...
                for i in 0..4 {
                    ...
                    let handle = tokio::spawn(async move {
                        ...
                        let dataset =
                            Dataset::from_path(&dataset_path).expect("Failed to load dataset");
...
pub fn bench_memory_usage(c: &mut Criterion) {
    ...
    for size in [10_000, 100_000, 1_000_000].iter() {
        group.bench_with_input(
            BenchmarkId::new("dataset_processing", size),
            size,
            |b, s| {
                b.iter(|| {
                    let generator = TestDataGenerator::new();
                    let dataset_path =
                        generator.create_medium_csv(&format!("memory_bench_{}", s), *s);
