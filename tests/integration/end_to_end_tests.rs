...
async fn test_complete_ml_pipeline_workflow() {
    ...
    let raw_dataset = Dataset::from_path(&raw_data_path).expect("Failed to load raw dataset");

    let raw_proof = ledger
        .notarize_dataset(raw_dataset, "raw-customer-data-v1", ProofConfig::default())
    ...
    let cleaned_dataset =
        Dataset::from_path(&cleaned_data_path).expect("Failed to load cleaned dataset");

    let transform_proof = ledger
    ...
    let train_dataset = Dataset::from_path(&train_data_path).expect("Failed to load train dataset");
    let test_dataset = Dataset::from_path(&test_data_path).expect("Failed to load test dataset");

    let split_proof = ledger
        .record_split(
            "cleaned-customer-data-v1",
            vec![
    ...
    assert!(ledger
        .verify_proof(&raw_proof)
        .expect("Raw proof verification failed"));
    assert!(ledger
        .verify_proof(&transform_proof)
        .expect("Transform proof verification failed"));
    assert!(ledger
        .verify_proof(&split_proof)
        .expect("Split proof verification failed"));
}
...
async fn test_concurrent_ledger_operations() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let mut ledger =
                Ledger::open(&ledger_path, LedgerConfig::default()).expect("Failed to open ledger");
    ...
    // Verify all proofs
    let ledger =
        Ledger::open(&ledger_path, LedgerConfig::default()).expect("Failed to open final ledger");

    for proof in proofs {
        assert!(ledger
            .verify_proof(&proof)
            .expect("Concurrent proof verification failed"));
    }
...
async fn test_large_dataset_processing() {
    ...
    let generator = TestDataGenerator::new();
    let large_dataset_path =
        generator.create_medium_csv("large_dataset", constants::LARGE_DATASET_ROWS);

    let timer = PerformanceTester::new();

    let dataset = Dataset::from_path(&large_dataset_path).expect("Failed to load large dataset");
    ...
    assert!(ledger
        .verify_proof(&proof)
        .expect("Large dataset proof verification failed"));
}
...
async fn test_error_handling_and_recovery() {
    ...
    let dataset = Dataset::from_path(&valid_path).expect("Failed to load valid dataset");

    // First notarization should succeed
    let _proof1 = ledger
        .notarize_dataset(dataset.clone(), "duplicate-name", ProofConfig::default())
        .expect("First notarization should succeed");

    // Second notarization with same name should fail
    let result = ledger.notarize_dataset(dataset, "duplicate-name", ProofConfig::default());
...
async fn test_export_formats_and_compatibility() {
    ...
    let _proof = ledger
        .notarize_dataset(dataset, "export-test-v1", ProofConfig::default())
        .expect("Failed to notarize dataset");

    // Test JSON-LD export
    let json_ld_report = ledger
        .generate_audit_report(None, None, true, "json-ld".to_string())
        .expect("Failed to generate JSON-LD report");

    // Validate JSON-LD structure
    let json_value: serde_json::Value =
        serde_json::from_str(&json_ld_report).expect("JSON-LD report should be valid JSON");
    assert!(
    ...
    // Test JSON export
    let json_report = ledger
        .generate_audit_report(None, None, true, "json".to_string())
        .expect("Failed to generate JSON report");

    let json_value: serde_json::Value =
        serde_json::from_str(&json_report).expect("JSON report should be valid JSON");
    assert!(
        json_value.get("datasets").is_some(),
        "Should have datasets field"
    );

    // Test HTML export (if implemented)
    let html_result = ledger.generate_audit_report(None, None, true, "html".to_string());

    match html_result {
        Ok(html_report) => {
            assert!(html_report.contains("<html>"), "Should contain HTML tags");
            assert!(
                html_report.contains("export-test-v1"),
...
async fn test_storage_backend_integration() {
    ...
    let rocks_proof = rocks_ledger
        .notarize_dataset(dataset, "rocks-test-v1", ProofConfig::default())
    ...
    if let Ok(db_url) = std::env::var("TEST_DATABASE_URL") {
        ...
        match pg_ledger_result {
            Ok(mut pg_ledger) => {
                ...
                let pg_proof = pg_ledger
                    .notarize_dataset(dataset, "pg-test-v1", ProofConfig::default())
...
async fn test_performance_under_load() {
    ...
    for i in 0..10 {
        ...
        let proof = ledger
            .notarize_dataset(dataset, &format!("load-test-{}", i), ProofConfig::default())
    ...
    for proof in proofs {
        assert!(ledger
            .verify_proof(&proof)
            .expect("Load test proof verification failed"));
    }
